/*
    DBOps - BackupV3-1 - Log

    Parametric SQL Agent job for LOG backups to Azure Blob Storage.
    All databases in the list get individual job steps.

    Deploy with: sqlcmd -S <server> -i V3-1-Log.sql [-v DatabaseList="db1,db2" StorageAccountName="myaccount" ...]
*/
...
:setvar JobEnabled 1
:setvar ScheduleStartTime 0

-- Derived constants (do not override)
:setvar JobName "DBOps - BackupV3-1 - Log - @Every15mins - Dur:--mins~"
:setvar ContainerPath "v3-1-weeklychain/logs"
:setvar ScheduleName "BACKUP LOG SCHEDULE EVERY 15 MINS"
...
EXEC @ReturnCode = msdb.dbo.sp_add_job
    ...
    @notify_level_page = 0,
    @delete_level = 0,
    @description = N'Parametric V3-1 Log backup job. Deployed via DBOps.',
...
-- Populate database list from online user databases (FULL recovery model only)
...
BEGIN
    ...
    SET @OnSuccessAction = CASE WHEN @i = @TotalSteps THEN 1 ELSE 3 END  -- last step: quit success; others: next step
